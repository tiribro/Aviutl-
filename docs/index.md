...
フィルタは４つだけですが、オプション設定がやたらと多いので、とりあえず高さ1080サイズの画面ならこれくらいだろうという設定画面のサンプルスクショも並べておきました。とりあえず何も考えずに同じ数字をぽちぽち入力していって、なんかできそうだなと思ったらあらためて解説を読んでください。また、一度設定してしまえば、テキスト（セリフ）を打ち直すだけでリサイズとかだいたい良い感じに勝手になるようしておきましたので、一度セットアップしたものをAviutlの「エイリアス」機能を使ってテンプレフキダシとしてセーブして量産すると使いやすいと思います
...
#### 設定＞アルファFadeIn／Out
> フェード秒数を指定します。0以上を指定した場合、透明度によるフェードを行います

#### 設定＞オフセットFadeIn／Out
> フェード秒数を指定します。0以上を指定した場合、X,Yオフセット量がフェードIn/Out時に動きます
